...
- TLS profile for secure DICOM transport: the `docker-compose.tls.yml` overlay switches the primary PACS to authenticated TLS (`dcmqrscp +tls`), with `scripts/gen-certs.sh` generating a self-signed CA + server + client certificates into a shared volume. `tests/test-tls.sh` verifies that a `+tls` C-ECHO succeeds and a plaintext association is refused, run by a dedicated CI job. `openssl` is added to the image. Host-verified end-to-end against a TLS dcmqrscp.
